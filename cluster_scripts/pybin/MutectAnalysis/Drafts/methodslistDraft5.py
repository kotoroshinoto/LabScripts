...
def outputTable(output_file_name, a_table):
	...
	for x in range(0, rows):
		for y in range(0, cols):
			...
			width = 1
			if len(a_table[x][y]) * 256 > width:
				width = len(a_table[x][y]) * 256
				sheet.col(x).width = width
			#sheet.col(x).width = len(a_table[x][y]) * 256
	...
	output = open(output_file_name, 'w') #Write table into output file
	output.truncate()
